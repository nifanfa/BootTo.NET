...
void I_FinishUpdate (void)
{
    ...
    y_offset     = ((s_Fb.yres - (SCREENHEIGHT * fb_scaling)) / 2) *
                   s_Fb.xres * s_Fb.bits_per_pixel / 8;
    ...
    // Center the 320x200 game image in output modes that have unused rows.
    // Clear those letterbox rows before writing the scaled image.
    memset(line_out, 0, y_offset);
    line_out += y_offset;
