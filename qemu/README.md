...
- License: GPL-2.0; see `COPYING`, `COPYING.LIB`, and `firmware/edk2-licenses.txt`

The directory contains `qemu-system-x86_64.exe`, its recursively resolved local DLL dependencies, x64 EDK2 firmware, and the ROM files used by the project. `firmware/edk2-i386-vars.fd` is the upstream variable-store template shared by the IA32 and X64 OVMF builds; the project copies it to the build output before QEMU starts so the bundled template remains unchanged. Other architecture emulators, unused QEMU utilities, non-x64 firmware, documentation, and development files from the installer were omitted.
