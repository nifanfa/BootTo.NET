...
namespace Playground.NES
{
    public class PPU
    {
        ...
        void CheckScanlineForSprites()
        {
            ...
            byte[] spriteToDrawSwap = spriteToDraw;
            spriteToDraw = spriteToDrawT;           // Stores the sprites used by the current scanline
            spriteToDrawT = spriteToDrawSwap;       // Reuse the previous buffer for the next scanline
        ...
        public PPU(MemoryMap memoryRef, Emulator tnRef)
        {
            memory = memoryRef;
            tn = tnRef;
