...
internal unsafe class NativeLib
{
    ...
    static extern int snprintf_(byte* buffer, int count, void* format, VariableArgument* va);

    [DllImport("*", EntryPoint = "vprintf_")]
    static extern int vprintf_(void* format, VariableArgument* va);

    public static int snprintf(byte* buffer, int count, void* format, params VariableArgument[] va)
    {
        fixed (VariableArgument* ptr = va)
        {
            return snprintf_(buffer, count, format, ptr);
        }
    }

    public static int vprintf(void* format, params VariableArgument[] va)
    {
        fixed (VariableArgument* ptr = va)
        {
            return vprintf_(format, ptr);
        }
    }
